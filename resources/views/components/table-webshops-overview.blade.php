...
<div class="ml-10 ">
    <table class="w-full ml-2">
        <thead>
        <tr>
            <th class="w-1/6 text-left">Webshop name</th>
            <th class="w-1/6 text-left">Owner name</th>
        ...
        @foreach ($objects as $object)
            <tr class="clickable" data-webshop-name="{{ $object->webshop->name }}"
                ...
                <td class="py-1 border-b">{{ $object->phonenumber }}</td><td class="py-1 border-b">
                    <form action="{{ route('webshop.edit', $object->id) }}" method="GET">
